function generateAsignments(){
    ...
    fetch("/asignments.json")
        ...
        .then((json) => {
            for (let i = 0; i < json.asignments.length; i++) {
                string += "<div>"
                    ...
                    string += "<div class=\"long\">"
                        ...
                        string += "<img src=\"" + json.asginments[i].images[j] + " \" style=\"width: calc(" + String(1 / json.asignments[i].images.length * 100 - 1) + "-10px)\">"
                        }
                    string += "</div>"
                    }else{
            ...
            document.getElementById("asignments").innerHTML = string;
        })
